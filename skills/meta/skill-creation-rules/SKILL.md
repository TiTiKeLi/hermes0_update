...
⚠️ **编号孤儿陷阱**：当在编号列表（如 `Step 3` 内部含 `3. ... 4. ...`）中插入新小节（如 `#### Step 3a`），旧的子编号会脱离上下文变成游离数字。修复方法：
- 插入前确认被插入段落的内部编号格式（是纯 Markdown 列表 `3.` 还是自动编号）
- 插入后 `read_file` 回读 → 检查与插入点相邻的 5 行是否出现悬空编号
- 如果在 `4.` 上方插入了 `#### Step 3a`，原 `4.` 若无父编号引导会变成孤立的编号行 → 改为无编号标题或用 `**n.**` 加粗
...
- 机械性约束（如果可以用 CI/校验强制执行——就自动化，文档留给需要判断的场景）
